...
test('keeps version and fork metadata aligned for the 1.6.0 release', () => {
  assert.equal(manifest.version, '1.6.0');
  assert.equal(packageJson.version, manifest.version);
  assert.equal(
    manifest.homepage_url,
    'https://github.com/yaisah/hide-promoted-jobs'
  );
  assert.equal(
    packageJson.repository.url,
    'git+https://github.com/yaisah/hide-promoted-jobs.git'
  );
  assert.equal(
    packageJson.bugs.url,
    'https://github.com/yaisah/hide-promoted-jobs/issues'
  );
});
